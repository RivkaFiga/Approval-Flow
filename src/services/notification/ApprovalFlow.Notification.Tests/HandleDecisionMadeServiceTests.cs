...
public class HandleDecisionMadeServiceTests
{
    ...
    public async Task Creates_projection_when_invoice_submitted_arrives_out_of_order()
    {
        ...
        Assert.Equal(LifecycleStatus.UnderReview, placeholder.CurrentStatus);
        Assert.Equal(Route.AutoApprove, placeholder.CurrentRoute);
        Assert.Equal(199.99m, placeholder.AmountUsd);
    }
